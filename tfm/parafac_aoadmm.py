...
@partial(jax.jit, backend=main_compute_device, static_argnums=(5, 6))
def aoadmm_update_no_lasso(UtM, UtU, x, dual_var, rho, tol, max_iter):
...
@partial(jax.jit, backend=main_compute_device, static_argnums=(6, 7))
def aoadmm_update_lasso(UtM, UtU, x, dual_var, rho, l1_reg, tol, max_iter):

    def cond(state):
        x, x_split, dual_var, iteration, x_old = state
        dual_residual = x - x_split
        primal_residual = x - x_old  # x_old
        return (iteration < max_iter) & (
            (jnp.linalg.norm(dual_residual) >= tol * jnp.linalg.norm(x)) |
            (jnp.linalg.norm(primal_residual) >= tol * jnp.linalg.norm(dual_var))
        )
    ...
    def body(state):
        ...
        x_new = soft_threshold(x_split.T - dual_var, threshold)
        # Dual variable update
        dual_var = dual_var + x_new - x_split.T
        return (x_new, x_split.T, dual_var, iteration + 1, x)
    
    x_split = x.copy()
    threshold = l1_reg / rho
    state = (x, x_split, dual_var, 0, x)
    x, x_split, dual_var, _, _ = jax.lax.while_loop(cond, body, state)
    return x, x_split, dual_var
...
def parafac_aoadmm(tensor: jnp.ndarray, 
                 rank: int, 
                 l1_reg: Tuple = (0, 0, 0), 
                 max_iter: int = 50, 
                 admm_tol: float = 1e-6,
    ...
    def body_fun(carry, _):
        factors, _ = carry
        # prev_reconstruction = reconstruct(weights, factors)
        for mode in range(n_modes):
            pseudo_inverse = jnp.ones((rank, rank))
            gram_matrices = jnp.array([factor.T @ factor for factor in factors])
            pseudo_inverse = jnp.prod(jnp.delete(gram_matrices, mode, axis=0), axis=0)
            rho = jnp.trace(pseudo_inverse) / rank
            
            # mttkrp = unfolding_dot_khatri_rao(tensor, factors, mode) # this might just be the bottleneck, try dropping mode here first
            selected = [f for i, f in enumerate(factors) if i != mode] 
            kr_factors = khatri_rao_product(selected)
            unfolded = jnp.moveaxis(tensor, mode, 0).reshape(tensor.shape[mode], -1)
            # mttkrp = unfolded @ jnp.conj(kr_factors)
            mttkrp = unfolded @ kr_factors

            if l1_reg[mode]:
                factors[mode], factors_aux[mode], duals[mode] = aoadmm_update_lasso(
                    UtM=mttkrp,
                    UtU=pseudo_inverse,
                    x=factors[mode],
                    dual_var=duals[mode],
                ...
                factors[mode], factors_aux[mode], duals[mode] = aoadmm_update_no_lasso(
                    UtM=mttkrp,
                    UtU=pseudo_inverse,
                    x=factors[mode],
                    dual_var=duals[mode],
        ...
        # new_reconstruction = reconstruct(weights, factors)
        # error = jnp.linalg.norm(new_reconstruction - prev_reconstruction) / jnp.linalg.norm(prev_reconstruction)
        return (factors, jnp.inf), jnp.inf

    # Run iterations
    init_carry = (factors, jnp.inf)
    (factors, _), _ = jax.lax.scan(body_fun, init_carry, None, length=max_iter)
    return weights, factors
...
def benchmark_comparison():
    ...
    """
    print("Running benchmark: AO-ADMM vs. ALS\n")
    ...
    parafac_aoadmm(X_log, rank, l1_reg)
    parafac_enhanced(X_log, rank)

    time_start = time.time()
    for _ in range(ITERATIONS):
        weights_admm, factors_admm = parafac_aoadmm(X_log, rank, l1_reg)
    time_end = time.time()
    ...
    print(f"AO-ADMM-based parafac relative reconstruction error: {error_admm:.10f}")
    print(f"ALS-based parafac relative reconstruction error: {error_als:.10f}")


if __name__ == "__main__":
    jax.config.update("jax_enable_x64", True)
    # os.environ['CUDA_VISIBLE_DEVICES'] = "2" # Titan and FP64 mode
