...
for lag in lst_lags:
    ...
    for K in lst_K: # can i speed this up using jax.lax.scan?
        ...
        #         model, naive, hor1, W, F, B, mv_tfm = Tensor_Models(X_log[:, :lag, :], jnp.arange(i, min(i + lag_to_chunk[lag], ub)), K, window_size, lag, max_horizon, None) # dim: (num_windows, args.max_horizon)
